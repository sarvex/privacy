...
def main(unused_argv):
  ...
  print(f"Epsilons (Noisy Max): {str(eps_list_nm)}")
  print(f"Smoothed sensitivities (Noisy Max): {str(total_ss_nm / l_list)}")
  ...
  ss_scale = 2.0 / ss_eps
  print(f"To get an {str(ss_eps)}-DP estimate of epsilon, ")
  print(f"..add noise ~ {str(ss_scale)}")
  print(f"... times {str(total_ss_nm / l_list)}")
  print(f"Epsilon = {str(min(eps_list_nm))}.")
  ...
  print(f"Data independent bound = {str(min(data_ind_eps_list))}.")
